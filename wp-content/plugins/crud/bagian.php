...
// Jika mode insert, dapatkan kdbag baru dari nilai maksimal yang ada di tabel
if ($MODE === "tambah") {
    $sql = "SELECT MAX(kdbag) AS max_kdbag FROM tbl_bagian";
    $result = $conn->query($sql);
    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $maxValue = $row['max_kdbag'];
        // Jika belum ada data, gunakan 1 sebagai nilai awal
        $newValue = $maxValue ? intval($maxValue) + 1 : 1;
        // Format dengan 2 digit, misalnya 5 menjadi "05"
        $kdbag = str_pad($newValue, 2, "0", STR_PAD_LEFT);
    } else {
        // Jika query gagal, defaultnya "01"
        $kdbag = "01";
    }
} else {
    // Mode edit, ambil dari data yang diedit
    $kdbag = isset($editData['kdbag']) ? $editData['kdbag'] : '';
}
// Ambil data dari database untuk ditampilkan di tabel
$result = mysqli_query($conn, "SELECT * FROM tbl_bagian");
?>

<h2 class="text-center">Manajemen Bagian</h2>
<form method="POST">
    <div class="mb-3 mt-3">
